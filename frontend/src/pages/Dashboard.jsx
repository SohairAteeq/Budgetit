...
// 🔹 helper to render icon (emoji from db OR fallback)
const renderIcon = (tx, fallbackIcon) => {
  if (tx.icon) {
    return (
    ...
    const fetchDashboard = async () => {
      ...
      try {
        ...
        setData({
          ...
          latestTransactions: resp.latestTransactions ?? [],
          last5Incomes: resp.last5Incomes ?? [],
          last5Expenses: resp.last5Expenses ?? [],
        });
      } catch (err) {
                  ...
                  <div className="max-h-64 overflow-y-auto pr-2">
                    {data.latestTransactions?.length > 0 ? (
                      <ul className="space-y-3">
                        {data.latestTransactions.slice(0, 20).map((tx) => (
                          <li
                            key={tx.id}
                        ...
                        >
                          {chartData.map((entry, i) => (
                            <Cell
                              key={`c-${i}`}
                              fill={
                                allZero ? "#6b7280" : COLORS[i % COLORS.length]
                              }
                            />
                          ))}
                        </Pie>
